...
ggplot(data = all_groups_topo_fnl) + 
  geom_density(
    mapping = aes(x = hli, fill = hli_overall_qrtl)
    , color = NA
  ) + 
  ...
  xlab("HLI") +
  ylab("Density") +
  scale_x_continuous(breaks = scales::extended_breaks(n=10), limits = c(min(all_groups_topo$hli)*0.98, 1.001) ) +
  scale_fill_viridis_d(option = "turbo", alpha = 0.9) +
  labs(
    title = "Heat Load Index distribution by cool-warm site type"
  ) +
  theme_bw() + 
  theme(
    legend.position = "top"
