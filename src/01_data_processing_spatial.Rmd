...
topo <- c(
  ...
  dplyr::mutate(
    slope_rad = pracma::deg2rad(as.numeric(slope))
    , folded_aspect = 180 - abs(as.numeric(aspect)-180)
    , folded_aspect_rad = pracma::deg2rad(folded_aspect)
  )

# apply median aspect and slope to vector
slope_units <- aggregate(topo %>% dplyr::select(slope_rad), units, median) # %>% sf::st_as_sf() %>% sf::st_set_geometry(NULL) %>% View()
aspect_units <- aggregate(topo %>% dplyr::select(folded_aspect_rad), units, median)
center_units <- sf::st_centroid(units)$geometry %>% 
  sf::st_transform(crs = 4326) %>% 
  sf::st_coordinates()

units <- units %>% 
  dplyr::mutate(
    slope_rad = slope_units$slope_rad
    , folded_aspect_rad = aspect_units$folded_aspect_rad
    , latitude_rad = pracma::deg2rad(as.numeric(center_units[,2]))
    , slope_deg = pracma::rad2deg(slope_units$slope_rad)
    , folded_aspect_deg = pracma::rad2deg(aspect_units$folded_aspect_rad)
    , latitude_deg = as.numeric(center_units[,2])
  )

units %>% sf::st_set_geometry(NULL) %>% View()
